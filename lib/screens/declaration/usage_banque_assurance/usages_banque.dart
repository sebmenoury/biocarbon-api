...
class UsagesBanqueScreen extends StatefulWidget {
  final String codeIndividu;
  final VoidCallback onSave;
  final String sousCategorie;
  final String valeurTemps;

  const UsagesBanqueScreen({super.key, required this.codeIndividu, required this.sousCategorie, required this.valeurTemps, required this.onSave});

  @override
  State<UsagesBanqueScreen> createState() => _UsagesBanqueScreenState();
}

class _UsagesBanqueScreenState extends State<UsagesBanqueScreen> {
  ...
  Future<void> loadData() async {
    ...
    final existants = postes.where((p) => p.sousCategorie == 'Banques et Assurances').toList();
    ...
    final refFiltree = ref.where((e) => e['Sous_Categorie'] == 'Banques et Assurances').toList();
  ...
  Future<void> supprimer() async {
    ...
    if (confirm == true) {
      ...
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(builder: (_) => PosteListScreen(typeCategorie: "Biens et services", sousCategorie: widget.sousCategorie, codeIndividu: widget.codeIndividu, valeurTemps: widget.valeurTemps)),
      );
    }
  }
      ...
      children: [
        if (isLoading)
          ...
          const Padding(
            ...
            child: Text(
              "⚙️ Indiquez ici le montant de cos comptes Banques et Assurances. \n"
          ...
          CustomCard(
            ...
            child: Column(
              ...
              children: [
                Text("Relevé de comptes", style: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold)),
